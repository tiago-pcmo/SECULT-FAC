(function () {
    ...
    angular
        .module("fac")
        .controller("alteracaoListagemController", ["$rootScope", "$scope", "$location", "$routeParams", "alteracaoService" ,"inscricaoService", "utilitariosService", "opcoes", "entidadeService",
            function ($rootScope, $scope, $location, $routeParams, alteracaoService, inscricaoService, utilitariosService, opcoes, entidadeService) {
                  $scope.alteracoesListagem = [];

                  alteracaoService.getList().then(function (response) {
                      $scope.alteracoesListagem = response.data;
                  });

                $scope.nova = function () {
                    $location.path("/alteracao/novo");
                };
                ...
                $scope.downloadDocumento = function (id, downloadCallback) {
                    alteracaoService.download({ id: id }).then(function (response) {
                        downloadCallback(response.data);
                    });
                };
            }])
        .controller("alteracaoAprovacaoController", ["$rootScope", "$scope", "$location", "$routeParams", "alteracaoService", "inscricaoService", "editalService", "utilitariosService", "opcoes", "entidadeService",
            function ($rootScope, $scope, $location, $routeParams, alteracaoService, inscricaoService, editalService, utilitariosService, opcoes, entidadeService) {
                $scope.opcoes = {};
                ...
                $scope.avaliar = function () {
                    $location.path("/alteracaoFicha/novo");
                };
                ...
                inscricaoService.getDisponiveisParaAnaliseAlteracao().then(function (response) {
                    if (response.data.length > 0) {
                        $scope.opcoes.listagemGeral = response.data.map(function (c) {
                            return $scope.newFicha(c.codigo, c.titulo, c.edital, c.codigoEdital, c.codigoModalidade, c.documentos, c.avaliados, c.aprovados, c.reprovados, c.extensao, c.mimeType, c.nomeCompleto, c.tamanho)
                        });
          ...
          .controller("alteracaoFichaController", ["$rootScope", "$scope", "$location", "$routeParams", "alteracaoService", "utilitariosService", "opcoes", "entidadeService",
            function ($rootScope, $scope, $location, $routeParams, alteracaoService, utilitariosService, opcoes, entidadeService) {
                ...
                $scope.aceite = function (id, observacao) {
                    alteracaoService.aceite({ id: id, observacao: observacao }).then(function (response) {
                        $scope.getAlteracoesAprovacao();
                    });
                };

                $scope.recuse = function (id, observacao) {
                    debugger;
                    alteracaoService.recuse({ id: id, observacao: observacao }).then(function (response) {
                        $scope.getAlteracoesAprovacao();
                    });
                };

                $scope.getAlteracoesAprovacao = function () {
                    alteracaoService.getAlteracoesAprovacao({ id: $routeParams.id }).then(function (response) {
                        if (response.data.length > 0) {
        ...
        .controller("alteracaoManutencaoController", ["$rootScope", "$scope", "$location", "opcoes", "$routeParams", "alteracaoService", "utilitariosService", "accountService", "inscricaoService", "notificacaoService", "entidadeService", "tipoDocumentoService",
            function ($rootScope, $scope, $location, opcoes, $routeParams, alteracaoService, utilitariosService, accountService, inscricaoService, notificacaoService, entidadeService, tipoDocumentoService) {
                ...
                alteracaoService.getTipos().then(function (response) {
                    $scope.tiposAlteracao = response.data;
                });

                if (opcoes.modoEdicao) {
                    alteracaoService.get({ id: $routeParams.id }).then(function (response) {
                ...
                $scope.salve = function () {
                    ...
                    if (opcoes.modoEdicao) {
                        alteracaoService.salvePut({ objeto: $scope.documento });
                        return;
                    }

                    alteracaoService.salvePost({ objeto: $scope.documento });
                };
