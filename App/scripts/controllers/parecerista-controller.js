(function () {
    ...
    angular
        .module("fac")
        .controller("pareceristaController", ["$scope", "$location", "entidadeService", "opcoes", "accountService", "utilitariosService",
                ...
                if (utilitariosService.possuiValor(conta)) {
                    if (conta.role === utilitariosService.roles.administrador) {
                        entidadeService.getLista({ entidade: opcoes.entidade }).then(function (response) {
                            $scope.pareceristas = response.data;
                        });
        ...
        .controller("pareceristaManutencaoController", ["$rootScope", "$scope", "$location", "$routeParams", "opcoes", "entidadeService", "editalService", "utilitariosService", "accountService", "inscricaoService",
            function ($rootScope, $scope, $location, $routeParams, opcoes, entidadeService, editalService, utilitariosService, accountService, inscricaoService) {
                    ...
                    if (conta.role === utilitariosService.roles.administrador) {
                        if ($scope.modoEdicao) {
                            entidadeService.get({ entidade: opcoes.entidade, id: $routeParams.id }).then(function (response) {
                                $scope.parecerista = response.data;
                            });
                        }

                        $rootScope.submitForm = function () {
                            var opcoesService = { entidade: opcoes.entidade, objeto: $scope.parecerista };
